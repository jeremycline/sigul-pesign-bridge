    ...
    loop {
        ...
        match unix_stream.read_exact(&mut buf).await {
            Ok(_bytes_read) => {
                ...
                let version = header.next().ok_or_else(|| {
                    anyhow!("Programmer error: the header should be of length 3")
                })??;
                ...
                match command {
                    Command::Kill => {
                        tracing::info!("Ignoring client command to halt the daemon");
                        Ok(())
                    }
                    Command::UnlockToken => todo!(),
                    Command::SignAttached => todo!(),
                    Command::SignDetached => todo!(),
                    Command::Response => todo!(),
                    Command::IsTokenUnlocked => {
                        is_token_unlocked(&mut unix_stream, payload_length).await
                    }
                    Command::GetCmdVersion => {
                        get_command_version(&mut unix_stream, payload_length).await
                    }
                    Command::SignAttachedWithFileType => {
                        sign_attached_with_filetype(&mut unix_stream, payload_length).await
                    }
                    Command::SignDetachedWithFileType => todo!(),
                }?;
            }
